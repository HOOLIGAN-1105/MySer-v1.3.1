...
<div id="client-modal-overlay" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:100000; justify-content:center; align-items:center;">
    <div style="background:#fff; border-radius:8px; padding:25px; width:650px; max-width:90%; max-height:90vh; overflow-y:auto; box-shadow:0 4px 20px rgba(0,0,0,0.3);">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
            <h3 id="client-modal-title" style="margin:0;">➕ <?php _e('Добавить клиента', 'myser'); ?></h3>
        ...
        <div style="margin-bottom:15px;">
            <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Тип клиента', 'myser'); ?></label>
            <select id="client-type" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;" onchange="myser_toggle_client_fields()">
                <option value="person"><?php _e('Физическое лицо', 'myser'); ?></option>
                <option value="company"><?php _e('Юридическое лицо', 'myser'); ?></option>
            </select>
        </div>

        <!-- Общие поля -->
        <fieldset style="border:1px solid #e0e0e0; border-radius:4px; padding:15px; margin-bottom:15px;">
            <legend style="font-weight:600; padding:0 10px;"><?php _e('Основная информация', 'myser'); ?></legend>
            
            <!-- Физлицо: ФИО -->
            <div id="client-person-name" style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:15px;">
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Фамилия', 'myser'); ?></label>
                    <input type="text" id="client-last-name" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                </div>
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Имя', 'myser'); ?> *</label>
                    <input type="text" id="client-first-name" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;" required>
                </div>
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Отчество', 'myser'); ?></label>
            ...
            <div id="client-company-name" style="display:none; grid-template-columns:1fr 1fr; gap:15px;">
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Название компании', 'myser'); ?> *</label>
                    <input type="text" id="client-company" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                </div>
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Форма собственности', 'myser'); ?></label>
                    <select id="client-legal-form" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                        <option value=""><?php _e('Не указано', 'myser'); ?></option>
                        <option value="ooo">ООО</option>
                        <option value="ip">ИП</option>
                        <option value="zao">ЗАО</option>
                        <option value="oao">ОАО</option>
                        <option value="other"><?php _e('Другое', 'myser'); ?></option>
                    </select>
                </div>
            </div>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px; margin-top:15px;">
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Телефон', 'myser'); ?></label>
                ...
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Доп. телефон', 'myser'); ?></label>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px; margin-top:15px;">
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Email', 'myser'); ?></label>
                ...
                <div>
                    ...
                    <label style="display:flex; align-items:center; gap:8px; padding-top:6px;">
                        ...
                        <span>⚠️ <?php _e('Проблемный клиент', 'myser'); ?></span>
                    </label>
                </div>
            </div>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:15px; margin-top:15px;">
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Город', 'myser'); ?></label>
                    <input type="text" id="client-city" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                </div>
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Улица', 'myser'); ?></label>
                    <input type="text" id="client-street" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                </div>
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Дом', 'myser'); ?></label>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px; margin-top:15px;">
                <div>
                    <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Скидка на услуги (%)', 'myser'); ?></label>
                    <div style="display:flex; gap:8px; align-items:center;">
                        ...
                        <button type="button" id="apply-suggested-discount" class="button button-small" style="white-space:nowrap; padding:4px 12px; background: #f0f0f0; border:1px solid #ccc; border-radius:4px; cursor:pointer;" title="<?php _e('Применить предложенную скидку', 'myser'); ?>">✅</button>
                    </div>
            ...
            <div style="margin-top:15px;">
                <label style="display:block; margin-bottom:5px; font-weight:600;"><?php _e('Заметки', 'myser'); ?></label>
                <textarea id="client-notes" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px; min-height:60px;" placeholder="<?php _e('Дополнительная информация...', 'myser'); ?>"></textarea>
            </div>
        </fieldset>

        <!-- Кнопки -->
        <div style="display:flex; gap:10px; justify-content:flex-end;">
            <button class="button" onclick="myser_close_client_modal()"><?php _e('Отмена', 'myser'); ?></button>
            <button class="button button-primary" onclick="myser_save_client_from_modal()"><?php _e('Сохранить', 'myser'); ?></button>
        </div>
    </div>
</div>
